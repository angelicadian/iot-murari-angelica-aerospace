...
void loop() {
  ...
  if (millis() - lastUpdateMillis >= 1000) {
    ...
    urusanIoT.publish("aerospace/murari/status", payload);
  }
...
void subscribe() {
  urusanIoT.subscribe("aerospace/murari/command");
}
...
void penangkapPesan(String topic, String message){
  ...
  if(galatParseJson == DeserializationError::Ok){
    ...
    !dataMasuk["humidUpperThreshold"].isNull()){
      tempBottomThreshold = dataMasuk["tempBottomThreshold"].as<int>();
      tempUpperThreshold = dataMasuk["tempUpperThreshold"].as<int>();
      humidBottomThreshold = dataMasuk["humidBottomThreshold"].as<int>();
      humidUpperThreshold = dataMasuk["humidUpperThreshold"].as<int>();
    }
  }
  else
  {
